...
RAW_OUTPUT_FILE = "mpnn_0129b1_mutations_raw.csv"
FINAL_SUMMARY_FILE = "mpnn_0129b1_mutations_summary.csv"
THREADS_TO_USE = 4
N_STRUCT = 25
...
if __name__ == "__main__":
    ...
    print(f"Starting ProteinMPNN Mutation Screen.")
    print(f"Total mutations: {len(mutations)}")
    print(f"Runs per mutation: {N_STRUCT}")
    ...
    if len(job_queue) > 0:
        with multiprocessing.Pool(processes=THREADS_TO_USE) as pool:
            for result in tqdm(pool.imap_unordered(perform_mutation_task, job_queue), 
                             total=len(job_queue), 
                             desc="Processing Mutants"):
