...
OUTPUT_CSV = "final_5T30_mutants_results.csv"
THREADS_TO_USE = 15
N_REPLICATES = 25
...
def main():
    ...
    for variant in COMBOS.keys():
        ...
        if var_df.empty: continue

        ddgs = var_df['ddG'].values
        q25, q75 = np.percentile(ddgs, [25, 75])
        iqm_vals = ddgs[(ddgs >= q25) & (ddgs <= q75)]
        
        iqm = np.mean(iqm_vals) if len(iqm_vals) > 0 else np.mean(ddgs)

        var_df = var_df.copy()
        var_df['Diff'] = abs(var_df['ddG'] - iqm)
        best_row = var_df.loc[var_df['Diff'].idxmin()]

        best_file = best_row['PDB_File']
        best_rep_id = best_row['Replicate']
        
        final_name = f"{variant}_Representative.pdb"
        final_wt_name = f"{variant}_WT_Representative.pdb"
        
        best_wt_file = f"{variant}_WT_rep{best_rep_id}.pdb"
        
        if os.path.exists(best_file):
            shutil.copy(best_file, final_name)
            print(f"Variant: {variant}")
            print(f"  IQM ddG: {iqm:.3f}")
            print(f"  Rep PDB: {best_file} (ddG: {best_row['ddG']:.3f})")
            print(f"  RMSD:    {best_row['Active_Site_RMSD']:.3f} Å")
            print(f"  Class:   {best_row['Classification']}")
            print(f"  Saved As: {final_name}")
            
        if os.path.exists(best_wt_file):
            shutil.copy(best_wt_file, final_wt_name)
            print(f"  Saved WT: {final_wt_name}")
            print("-" * 40)
